...
exports.searchBarber = async (req,res)=>{
    try{
        ...
        if(!text){
            return (
                res.json({
                    message: "Please Provide text to search barber",
                    status: false
                })
            )
        }

        const result  = await userModel.find({user_type:'barber' , user_name: {$regex : text}});
...
exports.view_block_barbers = async (req,res)=>{
    try{
        const result = await userModel.find({user_type:'barber' , blockStatus:true});
        if(result){
            res.json({
                message: "Fetched",
                status:true,
