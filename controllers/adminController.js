...
exports.getDashBoard = async(req,res)=>{
    try{
        ...
        const total_barbers = await userModel.count({user_type:'barber'});
        const total_customers = await userModel.count({user_type:'customer'});
        const totalHairStyles = await hairStyleModel.count();
        const totalAppointments = await appointmentModel.count();

        
        if(total_barbers){
            res.json({
                message: "Dash Borad data",
                status:true,
